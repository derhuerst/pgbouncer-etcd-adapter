...
const mapEtcdEntriesToPgbouncerIni = (rawEntries) => {
	debugPgbouncerIni('rawEntries', rawEntries)
	const pgbouncer = {}
	const nested = {
		pgbouncer: pgbouncer,
	}
	...
	debugPgbouncerIni('nested', nested)

	const data = encodeAsIni(nested, {
		newline: true,
...
const mapEtcdEntriesToUserlistTxt = (rawEntries) => {
	debugUserlistTxt('rawEntries', rawEntries)
	let data = ''
	for (const [dotKey, val] of Object.entries(rawEntries)) {
		const keys = dotKey.split('.')
		...
		if (keys.length > 2) {
			const err = GenerationError(`invalid key "${dotKey}", only one . is supported`)
			err.key = dotKey
			err.val = val
			throw err
		}
