...
const sortPgbouncerIniSections = (a, b) => {
	if (a === b) return 0
	if (a === 'pgbouncer') return -1
	if (b === 'pgbouncer') return 1
	return a < b ? 1 : -1
}
...
const mapEtcdEntriesToPgbouncerIni = (rawEntries) => {
	...
	pgbouncerIni = Object.fromEntries(
		Object.entries(pgbouncerIni)
		// sort sections
		.sort(([sectionA], [sectionB]) => sortPgbouncerIniSections(sectionA, sectionB))
		// sort each section's entries by key
		.map(([section, entries]) => [
			section,
			Object.fromEntries(
